...
ncrad <- read.csv(file.path(data_dir, "gene_count_NCRAD.txt"), sep = "\t", row.names = 1)
...
**Full Synthego RNAseq Data**

```{r}
datatable_download(synthego)
```

**Full NCRAD RNAseq Data**

```{r}
datatable_download(ncrad)
```

**Full NCRAD RNAseq Data from @bowles_development_2024**

```{r}
datatable_download(bowles)
```
