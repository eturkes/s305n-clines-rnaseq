...
packages <- c("edgeR", "ComplexHeatmap")
lapply(packages, library)
# --------------

# Define global settings.
# -----------------------
knitr::opts_chunk$set(fig.width = 7, fig.height = 10, dpi = 96)
# -----------------------
...
meta[[2]] <- data.frame(
  ...
  group = ifelse(sub(".*_(NNBB7|HCB8)_D\\d+$", "\\1", cols) == "NNBB7", yes = "WT", no = "S305N")
)
...
meta[[3]] <- data.frame(
  ...
  group = ifelse(sub(".*_(HCB8|NWBB7)_[68]W$", "\\1", cols) == "NWBB7", yes = "WT", no = "S305N")
)
...
## D21 (6W in Bowles Data) WT vs. HOM S305N
...
for (i in seq_along(data)) {
  tmp <- data[[i]][rownames(data[[i]]) %in% unlist(features), ]
  if (i != 3) {
    tmp <- tmp[ , grepl(timepoint, colnames(tmp)), drop = FALSE]
  } else {
    if (timepoint == "21") {
...
## D28 (8W in Bowles Data) WT vs. HOM S305N
