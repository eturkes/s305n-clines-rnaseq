---
title: "S305N Cell Lines RNAseq HET Analysis"
author:
...
`%notin%` <- Negate(`%in%`) # Create the %notin% operator.

# Add paths.
# ----------
data_dir <- file.path("..", "data")
cache_dir <- file.path("..", "cache", "HET")
...
match <- paste(
  c(
    "WT21", "WT28", "D421", "D428",
    "NNBB7_D21", "NNBB7_D28", "30012_D21", "30012_D28",
    "NWBB7_6W", "NWBB7_8W", "NW1B10_6W", "NW1B10_8W"
  ),
  collapse = "|"
)
data <- lapply(
...
meta[[2]] <- data.frame(
  ...
  genotype = factor(sub(".*_(NNBB7|a30012)_D\\d+$", "\\1", cols)),
  dataset = rep("NCRAD", times = length(cols)),
  group = ifelse(sub(".*_(NNBB7|a30012)_D\\d+$", "\\1", cols) == "NNBB7", yes = "WT", no = "S305N")
)
cols <- colnames(data[[3]])
meta[[3]] <- data.frame(
  sample = cols,
  timepoint = factor(sub(".*_(6W|8W)$", "\\1", cols)),
  genotype = factor(sub(".*_(NWBB7|NW1B10)_[68]W$", "\\1", cols)),
  dataset = rep("Bowles NCRAD", times = length(cols)),
  group = ifelse(sub(".*_(NWBB7|NW1B10)_[68]W$", "\\1", cols) == "NWBB7", yes = "WT", no = "S305N")
)
...
if (file.exists(cache)) {
  ...
  for (i in seq_along(data)) {
    ...
    dge <- DGEList(data[[i]])
    keep <- filterByExpr(dge, group = meta[[i]]$genotype)
    dge <- dge[keep, ]
    dge <- calcNormFactors(dge)
    data[[i]] <- DGEList(data[[i]])
    data[[i]]$samples$norm.factors <- dge$samples$norm.factors
    keep <- filterByExpr(data[[i]], group = meta[[i]]$genotype)
    ...
    if (i == 1) {
      ...
      cont_mat <- makeContrasts(
        a30012_vs_NNBB7_21 = a30012_21 - NNBB7_21,
        a30012_vs_NNBB7_28 = a30012_28 - NNBB7_28,
        levels = design
      )
    } else if (i == 3) {
      cont_mat <- makeContrasts(
        NW1B10_vs_NWBB7_6W = NW1B10_6W - NWBB7_6W,
        NW1B10_vs_NWBB7_8W = NW1B10_8W - NWBB7_8W,
        levels = design
      )
    }
...
colour2 <- c("lightgray", "darkgray")
names(colour1) <- levels(meta_comb$dataset)
names(colour2) <- levels(meta_comb$group)
```

## D21 (6W in Bowles Data) WT vs. HET S305N
...
for (i in 1:2) {
  features[[i]] <- features[[i]][features[[i]] %notin% features[[3]]]
}
```
...
mat <- mat[, c(1:4, 5, 7, 9, 6, 8, 10, 12, 11)]
mat_full <- mat

features <- list(features[[3]], features[[1]], features[[2]])
...
draw(
  Heatmap(
    ...
    row_split = factor(
      c(
        rep("DEG in Both Lines", times = length(features[[1]])),
        rep("DEG in Synthego Only", times = length(features[[2]])),
        rep("DEG in NCRAD Only", times = length(features[[3]]))
      ),
      levels = c("DEG in Both Lines", "DEG in Synthego Only", "DEG in NCRAD Only")
    ),
...
df$Genotype <- meta_comb_sub$group
...
mat_full <- mat_full[, seq_len(ncol(mat_full) - 2)]
...
## D28 (8W in Bowles Data) WT vs. HET S305N
